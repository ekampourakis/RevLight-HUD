...
#define ELM_SERIAL SoftwareSerial

ELM_SERIAL ELM_PORT(TX_PIN, RX_PIN);
...
bool Elm327::WaitOK(String Command, int Tries) {
  delay(100);
  ...
  while (OKFlag != true) {
    if (ELM_PORT.available()) {
      Resp[1] = Resp[0];
      Resp[0] = ELM_PORT.read();
	  DEBUG_SERIAL.print("Got: ");
	  DEBUG_SERIAL.println(Resp[0]);
    }    
    if (Resp[0] == 75 && Resp[1] == 79) {
      delay(1);
...
byte Elm327::connectBluetooth() {
	#ifdef DEBUG
		DEBUG_SERIAL.println();
		DEBUG_SERIAL.println("Starting bluetooth...");
		DEBUG_SERIAL.println("Pinging bluetooth...");
	#endif
	if (WaitOK("AT", 5) == true) {
		#ifdef DEBUG
			DEBUG_SERIAL.println("Connection OK");
		#endif
	} else {
		DEBUG_SERIAL.println("Connection no ping");
		return BLUETOOTH_NOPING;
	}
	#ifdef DEBUG
		DEBUG_SERIAL.println("Initializing bluetooth...");
	#endif
	if (WaitOK("AT+INIT", 5) == true) {
		#ifdef DEBUG
			DEBUG_SERIAL.println("Bluetooth initialized...");
		#endif
	} else {
		return BLUETOOTH_INIT_FAILED;
	}
	#ifdef DEBUG
		DEBUG_SERIAL.println("Binding bluetooth...");
	#endif
	if (WaitOK((String)"AT+BIND=" + ELM_MAC, 5) == true) {
		#ifdef DEBUG
			DEBUG_SERIAL.println("Bluetooth binded...");
		#endif
	} else {
		return BLUETOOTH_BIND_FAILED;
	}
	#ifdef DEBUG
		DEBUG_SERIAL.println("Pairing bluetooth...");
	#endif
	if (WaitOK((String)"AT+PAIR=" + ELM_MAC + ",20", 25) == true) {
		#ifdef DEBUG
			DEBUG_SERIAL.println("Bluetooth paired...");
		#endif
	} else {
		return BLUETOOTH_PAIR_FAILED;
	}
	#ifdef DEBUG
		DEBUG_SERIAL.println("Linking bluetooth...");
	#endif
	if (WaitOK((String)"AT+LINK=" + ELM_MAC, 10) == true) {
		#ifdef DEBUG
			DEBUG_SERIAL.println("Bluetooth linked...");
		#endif
	} else {
		return BLUETOOTH_LINK_FAILED;
	}
	return BLUETOOTH_SUCCESS;
}

//is blocking
//is lacking bt reset pin & function
byte Elm327::begin(){
	ELM_PORT.begin(ELM_BAUD_RATE);
	ELM_PORT.setTimeout(100);
	delay(500);
	#ifdef DEBUG 
		DEBUG_SERIAL.begin(DEBUG_BAUD_RATE);
		DEBUG_SERIAL.println("Communication initialized...");
	#endif
	byte Tmp;
	while (Tmp != BLUETOOTH_SUCCESS) {
		DEBUG_SERIAL.println("Bluetooth not connected... Resetting...");
		//ResetBT();
		Tmp = connectBluetooth();
	}
	delay(200);
	char data[20];
	DEBUG_SERIAL.println("Done bluetooth");
StartELM:
	//run all required tests for elm327 connection
	if (runCommand("ATD", data, 20) != ELM_SUCCESS) {
		DEBUG_SERIAL.println("Can't ATD");
	}
	delay(100);
	if (runCommand("ATZ", data, 20) != ELM_SUCCESS) {
		DEBUG_SERIAL.println("Can't ATZ");
	}
	delay(100);
	if (runCommand("ATE0", data, 20) != ELM_SUCCESS) {
		DEBUG_SERIAL.println("Can't ATE0");
	}
	delay(100);
	if (runCommand("ATL0", data, 20) != ELM_SUCCESS) {
		DEBUG_SERIAL.println("Can't ATL0");
	}
	delay(100);
	if (runCommand("ATS0", data, 20) != ELM_SUCCESS) {
		DEBUG_SERIAL.println("Can't ATS0");
	}
	delay(100);
	if (runCommand("ATH0", data, 20) != ELM_SUCCESS) {
		DEBUG_SERIAL.println("Can't ATH0");
	}
	delay(100);
	if (runCommand("ATSP0", data, 20) != ELM_SUCCESS) {
		DEBUG_SERIAL.println("Can't ATSP0");
		return ELM_PROTOCOL;
	}
	delay(2000); //delay some time for protocol to be established
	int TestInt;
	if (engineRPM(TestInt) != ELM_SUCCESS) {
		DEBUG_SERIAL.println((String)"Can't GETRPM 1" + engineRPM(TestInt));
		//return ELM_UNABLE_TO_CONNECT;
	}
	delay(5000);
	if (engineRPM(TestInt) != ELM_SUCCESS) {
		DEBUG_SERIAL.println((String)"Can't GETRPM 2" + engineRPM(TestInt));
		DEBUG_SERIAL.println("Can't connect to car ECU... Resetting PROTOCOL...");
		//ResetELM();
		goto StartELM;
		//return ELM_UNABLE_TO_CONNECT;
	}
...
byte Elm327::engineRPM(int &rpm){
	...
	DEBUG_SERIAL.println((String)"Took: " + (millis() - T));
	//DEBUG_SERIAL.println(values);
...
byte Elm327::getBytes( const char *mode, const char *chkMode, const char *pid, byte *values, unsigned int numValues){
	...
	DEBUG_SERIAL.println((String) "RPM: " + data);
	// Check the mode returned was the one we sent
	if ( data[0]!=chkMode[0] 
	  or data[1]!=chkMode[1]
	  or data[2]!=pid[0]
	  or data[3]!=pid[1] ){
		//DEBUG_SERIAL.println((String)"ELM sent: " + data[0] + data[1] + data[2] + data[3]);
		//DEBUG_SERIAL.println((String)"ELM note: " + chkMode[0] + chkMode[1] + pid[0] + pid[1]);
		return ELM_GARBAGE;
	}
...
{	
	...
	flush();
	ELM_PORT.print(cmd);
	ELM_PORT.print('\r'); //maybe i want /r/n???? !!!!!!
